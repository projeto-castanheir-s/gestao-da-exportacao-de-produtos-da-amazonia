...
import { ArrowLeft, Building, Plus, Edit, Trash2 } from 'lucide-react';

export default function EmpresasCastanheirasPage() {
  ...
  const columns = [
    ...
    },
    {
      ...
      label: 'CNPJ',
      render: (row) => (
        <div className="text-gray-700">{row.cnpj}</div>
      )
    },
    {
      key: 'estado',
      label: 'Estado',
      render: (row) => (
        <div className="text-gray-700">{row.estado}</div>
      )
    },
    {
      key: 'localExtracao',
      label: 'Local Extração',
      render: (row) => (
        <div className="text-gray-600 text-sm">{row.localExtracao}</div>
      )
    },
    {
      key: 'contato',
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-6 sm:py-8">
        ...
        <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-3xl sm:text-4xl font-bold text-menuGreen mb-2">
              Empresas Castanheiras
            </h1>
            <p className="text-gray-600 text-sm sm:text-base">
              Gerencie as empresas produtoras de castanha
            </p>
          </div>
          <button
            onClick={() => router.push('/cadastros/empresasCastanheiras/nova')}
          ...
          <Card className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{empresasCastanheiras.length}</p>
              </div>
            </div>
          </Card>
        </div>
        ...
        {/* Table */}
        <Card title="Lista de Empresas Castanheiras">
          <DataTable
            columns={columns}
            data={empresasCastanheiras}
          />
        </Card>
      </div>
    </main>
